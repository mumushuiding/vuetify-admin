...
export function passwordValid (rule, value, callback) {
  if (value === '' || value === undefined || value === null) {
    ...
    if ((!reg.test(value))) {
      callback(new Error('密码包含大写字母、小写字母、数字、特殊符号中的至少三类，且长度在8到20之间'))
    } else {
      callback()
    }
  }
...
// 验证是否是浮点
export function isDouble (rule, value, callback) {
  if (!value) {
    return callback(new Error('输入不可以为空'))
  }
  setTimeout(() => {
    if (!Number(value) && value !== '0') {
      callback(new Error('请输入[数字]'))
    } else {
